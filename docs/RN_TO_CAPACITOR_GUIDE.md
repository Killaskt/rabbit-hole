...
### ⚠️ Known Codemagic gotchas — read before first build

**1. No CocoaPods. No `.xcworkspace`. Ever.**
Capacitor v6+ uses Swift Package Manager (SPM). No `Podfile` exists and no `.xcworkspace` will be created.
- Always use `--project ios/App/App.xcodeproj` — never `--workspace`
- Never add a `pod install` step — there is no Podfile; it will fail or silently do nothing
- Error `Path "ios/App/App.xcworkspace" does not exist` → you have the wrong flag

**2. `cap add ios` requires macOS.** Cannot run on Windows or Linux.
The yaml above guards this with `if [ ! -d "ios" ]` so it only runs once per fresh clone.
Do **not** commit the `ios/` directory — it contains machine-generated absolute paths and must be regenerated by Codemagic on each build.

**3. `xcode-project use-profiles` must receive `--project` explicitly.**
Without it, the tool searches from the repo root and cannot locate `ios/App/App.xcodeproj`. The provisioning profile is never injected, and the archive step fails with `"App" requires a provisioning profile`. The fix is already in the yaml above: `xcode-project use-profiles --project "$XCODE_PROJECT"`
...
| `Path "ios/App/App.xcworkspace" does not exist` | Capacitor v6+ uses SPM — never use `--workspace`. Use `--project ios/App/App.xcodeproj` |
| `"App" requires a provisioning profile` | `xcode-project use-profiles` called without `--project "$XCODE_PROJECT"` — add the flag |
| First TestFlight build blocked on export compliance | No encryption declaration in Info.plist | Add `<key>ITSAppUsesNonExemptEncryption</key><false/>` to `ios/App/App/Info.plist` |
